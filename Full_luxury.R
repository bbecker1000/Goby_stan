...
#Goby model
  ...
  c(a_Goby, a_Breach, a_DO, a_SB, a_SC, a_SAV, a_Temp,
    ...
    tau ~ exponential(1),
    phi ~ dexp(100), #for dgampois if = 1 lambda goes negative
    c(SC_phi, SB_phi) ~ dexp(100) 
), 
data=dat , chains=4 , cores=4 , iter=500 , cmdstan=TRUE)
    # constraints=list(lambda ="lower=0.5 ,upper=5", SC_labmda="lower=0.3 ,upper=5",
    #                  phi ="lower=0", SB_phi="lower=0"))  ## to keep negbin value from going <0
    # constraints=list(a_Goby="lower=0"))
#beepr::beep(0)
...
Goby.m2.no.year.DAG.no_negatives <-  ulam(
  alist(
    #Goby model
    Goby ~ dgampois( mu, exp(phi)), #  #from dgampois help # note exp(log_scale) to constrain scale to positive reals
    # Goby ~ dgampois( lambda+100, phi),  # adding constant to lambda results in less negatives
    #Goby ~ poisson(lambda),
    ...
    tau ~ exponential(1),
    #phi ~ dexp(100), #for dgampois if = 1 lambda goes negative
    phi ~ dnorm(1,10), #from dgampois help
    c(SC_phi, SB_phi) ~ dexp(100) 
  ), 
  data=dat , chains=4 , cores=4 , iter=500 , cmdstan=TRUE
)

...
dat <- data.frame(Goby=Goby, Year=Year, SAV=SAV, SB=SB, SB_count=SB_count, SC=SC, 
...
#Effect of Rain on Breach
quantile( with(post, beta_Rain*beta_Breach_count)) ## ns
#effect of breach on DO
quantile( with(post, beta_DO*beta_Breach_count)) ## ns
#effect of SAV on SC
quantile( with(post, beta_SAV*beta_SC_count))  ## neg

quantile( with(post, beta_Temp*beta_SAV))  ## postive

quantile( with(post, beta_DO*beta_Temp)) ## -0.002

quantile( with(post, beta_Wind, beta_DO)) ## positive

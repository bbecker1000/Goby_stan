...
#test lmer
m1.lmer <- glmer(Goby ~ Year + Rain + SC + SAV + SB + DO + Temp + Breach + 
                Substrate +(1|Zone), data = dat, family = negative.binomial(0.6))
summary(m1.lmer)

library(rstanarm)
m1.stan_glm <- stan_glmer(Goby ~ Year + Rain + SC + SAV + SB + DO + Temp + Breach + 

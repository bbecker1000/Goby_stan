...
#same as 4a, but differing the regression coeffients to see if robust to other values...
...
# Define beta coefficients / 4a_simulation coefficients commented out.
betas <- c(
  Year = -0.25, #0.25,
  Year2 = -0.5, #-0.9,
  SB = 1.0, #0.9,
...
#need to add some dummy data not used but named in the stan model (holdover that I never cleaned up)
...
summary(model.glmmTMB.sim2)


#try lme4
model.glmer.sim2 <- glmer(
  Goby ~ 
    Year +  
    Year_2 +
    SB_count + 
...
summary(model.glmer.sim2)

# brms model
# asks for additional installations, say yes, and then takes ~60 seconds to start sampling.
library(brms)
model.brms.sim2 <- brm(
  Goby ~ 
    Year +  
...
#compile stan model if not already done
# write to stan file
...
fit.sim.sim2 <- mod$sample(  # model from 2B_stan_code.R.
  data = list_df_sim,
  seed = 123,
  chains = 3,
  iter_warmup = 2000,
...
rstan_fit.sim.sim2 <- read_stan_csv(fit.sim.sim2$output_files())
fit.sim.sim2$diagnostic_summary()

# for saving and seeing
print(fit.sim.sim2$summary(), n = 30)
...
ggsave("Output/sim2_forest.png", width = 20, height = 30, units = "cm")

cowplot::plot_grid(p.sim1, p.sim2,
                   labels = "AUTO")

ggsave("Output/sim1+2_forest.png", width = 40, height = 30, units = "cm")
